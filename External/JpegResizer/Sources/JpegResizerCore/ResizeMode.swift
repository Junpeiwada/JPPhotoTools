...
public enum ResizePlanner {
    ...
    /// 呼び出し側は nil のとき等倍経路をそのまま通す（再サンプルを避ける）。
    ///
    /// 出力寸法は **偶数** に丸める。JPEG には 4:2:0 クロマ由来の偶数寸法制約はないが、
    /// GainForge の挙動をそのまま踏襲して差分を最小化する（実害はない）。1px 未満の縦横比ずれは許容。
