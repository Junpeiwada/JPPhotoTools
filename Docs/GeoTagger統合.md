...
### 3.1 EXIF 読み書き: exiftool → ImageIO
...
**書き込み**: `CGImageDestinationCopyImageSource` を使う。これは**画像データを再エンコードせず**
メタデータだけ差し替えてコピーする API で、exiftool の「画素無劣化でタグ更新」と同等。

- `kCGImageDestinationMergeMetadata: true` ＋ `CGImageMetadata`（可変）で
  GPS / 日時タグのみを既存メタデータにマージし、MakerNotes 等は保持する
- 一時ファイルへ書き出し → `FileManager.replaceItemAt` で原子的に置換
  （元アプリの `-overwrite_original` 相当。バックアップファイルは作らない）
- 書き込むタグ（元実装と同一）:
...
**互換リスクは §7 の検証計画（exiftool との出力比較テスト）で潰す。**
...
| リスク | 影響 | 検証・対策 |
|--------|------|-----------|
| **ImageIO 書き込みの互換性**（最大のリスク） | MakerNotes 消失・メタデータ破損 | GT1 で同一 JPEG に「exiftool 書き込み」と「ImageIO 書き込み」を行い、`exiftool -j -a -G1` の全タグ出力を diff。画素データのハッシュ一致（無再圧縮）も確認。差分が許容できなければ書き込みだけ exiftool に戻す（読み取りはネイティブ維持） |
...
- 4 タブ目「ジオタグ」がビルド・起動し、GPX 読み込み → マッチング → 地図/テーブル確認 →
  一括付与の一連が実データで動作する
- ImageIO 書き込み後の JPEG が exiftool 検証で「GPS・日時タグが正しく、他タグと画素が無傷」
- GeoTaggerCore のユニットテスト（マッチング分岐・GPX パース・EXIF 往復・TZ 変換）が緑
- §5.3 チェックリストが全項目確認済み
- 外部依存（brew exiftool・ネットワーク地図タイル以外のオンライン API）なしで動作する
